...
public class SimplePicture implements DigitalPicture {
  ...
  public SimplePicture(SimplePicture copyPicture) {
    ...
    if (copyPicture.bufferedImage != null) {
      this.bufferedImage =
        new BufferedImage(copyPicture.getWidth(), copyPicture.getHeight(), BufferedImage.TYPE_INT_RGB);
  ...
  public Pixel[] getPixels() {
    ...
    for (int row = 0; row < height; row++) for (int col = 0; col < width; col++) pixelArray[row * width + col] =
  ...
  public Pixel[][] getPixels2D() {
    ...
    // loop through height rows from top to bottom
    for (int row = 0; row < height; row++) for (int col = 0; col < width; col++) pixelArray[row][col] =
      new Pixel(this, col, row);
